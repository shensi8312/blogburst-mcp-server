...
export const tools: Tool[] = [
  ...
  },
  {
    name: "get_seo_audit",
    description:
      "Get the latest SEO audit for your product — scores, technical issues, content gaps, and actionable recommendations.",
    inputSchema: {
      type: "object" as const,
      properties: {
        product_id: { type: "number", description: "Product ID (optional, uses default)" },
      },
    },
  },
  {
    name: "get_geo_audit",
    description:
      "Get the latest GEO (Generative Engine Optimization) audit — optimize for AI search engines like ChatGPT, Perplexity, Google AI Overviews.",
    inputSchema: {
      type: "object" as const,
      properties: {
        product_id: { type: "number", description: "Product ID (optional, uses default)" },
  ...
  },
  {
    ...
    description:
      "Get community engagement opportunities — HackerNews, Reddit, and forum threads where your product is relevant. Includes suggested replies.",
    inputSchema: {
      type: "object" as const,
      properties: {
  ...
  },
  {
    name: "get_diagnostic",
    description:
      "Get AI growth diagnostic — comprehensive marketing health check with scores for content quality, posting consistency, audience engagement, and growth rate.",
    inputSchema: {
      type: "object" as const,
      properties: {
        product_id: { type: "number", description: "Product ID (optional, uses default)" },
  ...
  },
  {
    name: "get_tasks",
    description:
      "Get marketing tasks generated by the AI CMO — SEO content, engagement tasks, optimization actions. Each task has prefilled content ready to execute.",
    inputSchema: {
      type: "object" as const,
      properties: {
        status: { type: "string", enum: ["pending", "in_progress", "completed"], description: "Filter by status (default: pending)" },
        limit: { type: "number", description: "Number of tasks to return (default: 10)" },
      },
    },
  },
  {
    name: "complete_task",
    description:
      "Mark a marketing task as completed.",
    inputSchema: {
      type: "object" as const,
      properties: {
...
export async function handleToolCall(
  ...
  try {
    ...
    switch (name) {
      ...
      case "get_seo_audit":
        result = await get(`/assistant/seo-audit${args.product_id ? `?product_id=${args.product_id}` : ""}`);
        break;

      case "get_geo_audit":
        result = await get(`/assistant/geo-audit${args.product_id ? `?product_id=${args.product_id}` : ""}`);
        break;

      case "get_opportunities": {
        const oppParams = new URLSearchParams();
        if (args.limit) oppParams.set("limit", String(args.limit));
        const oppQuery = oppParams.toString();
        result = await get(`/assistant/opportunities${oppQuery ? `?${oppQuery}` : ""}`);
        break;
      }

      case "get_diagnostic":
        result = await get(`/assistant/diagnostic${args.product_id ? `?product_id=${args.product_id}` : ""}`);
        break;

      case "get_tasks": {
        const taskParams = new URLSearchParams();
        if (args.status) taskParams.set("status", args.status as string);
        if (args.limit) taskParams.set("limit", String(args.limit));
        const taskQuery = taskParams.toString();
        result = await get(`/assistant/tasks${taskQuery ? `?${taskQuery}` : ""}`);
        break;
      }

      case "complete_task":
        result = await post(`/assistant/tasks/${args.task_id}/complete`, {});
        break;
